...
module.exports = function (RED) {
    function ReolinkEventNode(config) {
        ...
        node.lastEventStates = null;

        // Fetch data and send to output
        async function queryStates() {
            try {
                const eventStates = await server.queryCommand("GetEvents");
                if (eventStates && JSON.stringify(eventStates) !== JSON.stringify(node.lastEventStates)) {
                    node.send({
                        payload: eventStates,
                        topic: config.topic || server.name
                    });
                    node.lastEventStates = eventStates;
                }
